...
class SaveImageToDb {
  ...
  readImageData () {
    ...
    this.insertIndex = 0
    setTimeout(function () {
      that.saveDataInsertDb(that.insertIndex)
  ...
  //迭代进行数据存储 以免数据量太大导致 数据存储出现问题
  saveDataInsertDb (tempInsertIndex) {
    //一次存储1000个
    if (tempInsertIndex >= this.fileArr.length) {
      return
    }
    let startIndex = tempInsertIndex;
    let endIndex = tempInsertIndex + this.threadIndex;
    if (endIndex > this.fileArr.length) {
      endIndex = this.fileArr.length
    }
    let tempFileArr = [];
    for (let i = startIndex; i < endIndex; i++) {
      tempFileArr.push(this.fileArr[i])
    }
    this.saveAllPic(tempFileArr)
  }
  ...
  insertDataPl (dataArr) {
    ...
    this.dbServer.insertWork(parmsurl, dataArr).then((res) => {
      ...
      this.insertIndex += this.threadIndex;
      that.saveDataInsertDb(this.insertIndex)
      // dataArr=null
    }, (err) => {
      console.log(err, 1)
    });
  ...
  //数据加密  后续再加
  encryption (dataArr) {
    //替换6位数据 加上一个数值进行存储   不能随便修改 不然后续不能进行数据解译
    if (!this.encryptionArr || !this.encryptionIndexArr) {
      return
    }
    let indexArr = this.encryptionArr
    for (let i = 0; i < indexArr.length; i++) {
      dataArr[indexArr[i]] += this.encryptionIndexArr[i];
    }
  }
